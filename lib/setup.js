...
export async function setupProject(config) {
    ...
    console.log(chalk.cyan(`\n🚀 Initializing MAI Agents in ${targetDir}...`));
    ...
    try {
        ...
        if (fs.existsSync(templateDir)) {
            // In integration mode, we must be careful not to overwrite
            // fs.copy with overwrite: false might be safer, but we can't easily filter
            // So we manually copy key folders
            const foldersToCopy = ['.agents', 'workflows', 'config', 'skills'];
            for (const folder of foldersToCopy) {
                const srcPath = path.join(templateDir, folder);
                const destPath = path.join(targetDir, folder);
                if (fs.existsSync(srcPath)) {
                    // We allow overwriting agent/workflow files as they are the "core" of MAI
                    // But we should warn or backup if needed. For now, we assume user ran init to get these.
        ...
        if (!fs.existsSync(memoryPath)) {
            let memoryContent = `# 🧠 MAI MEMORY - ACTIVE CONTEXT
...
`;
            ...
            console.log(chalk.yellow('ℹ️ MAI_MEMORY.md already exists. Skipping creation.'));
        }
        ...
        const readmeContent = `# 🤖 MAI Agents Project: ${config.projectName}
...
`;
        ...
        if (fs.existsSync(readmePath)) {
            console.log(chalk.yellow('⚠️ README.md exists. Creating MAI_README.md instead.'));
